...
async function rebuildCdnCache(env2) {
  const { results } = await env2.DB.prepare("SELECT * FROM slots ORDER BY slot_number ASC").all();
  const payload = {
  ...
  await env2.BUCKET.put("grid.json", JSON.stringify(payload), {
    httpMetadata: { contentType: "application/json", cacheControl: "public, max-age=10" }
  });
}
__name(rebuildCdnCache, "rebuildCdnCache");
app.get("/api/grid", async (c) => {
  const file = await c.env.BUCKET.get("grid.json");
  if (!file) return c.json({ error: "CDN Cache Empty. Initialize via /api/init" }, 503);
...
app.post("/api/create-order", async (c) => {
  ...
  const slot = await c.env.DB.prepare("SELECT price FROM slots WHERE slot_number = ?").bind(slotId).first();
  ...
  const response = await fetch("https://api.razorpay.com/v1/orders", {
    ...
    body: JSON.stringify({
      ...
      // Amount in paise
      currency: "INR",
      receipt: `pyramid_slot_${slotId}`
    })
  });
  return c.json(await response.json());
});
app.post("/api/purchase", async (c) => {
  const { slotId, razorpay_order_id, razorpay_payment_id, razorpay_signature, userData } = await c.req.json();
  const isValid = await verifySignature(razorpay_order_id, razorpay_payment_id, razorpay_signature, c.env.RAZORPAY_KEY_SECRET);
  if (!isValid) return c.json({ error: "Security Check Failed" }, 401);
  ...
  if (result.meta.changes === 0) return c.json({ error: "Slot already taken" }, 409);
  c.executionCtx.waitUntil(rebuildCdnCache(c.env));
  return c.json({ success: true, message: "Pyramid updated" });
});
app.get("/api/init", async (c) => {
  if (c.req.query("key") !== c.env.ADMIN_SECRET) return c.text("Unauthorized", 401);
  await c.env.DB.exec(`
    CREATE TABLE IF NOT EXISTS slots (
      slot_number INTEGER PRIMARY KEY,
      price REAL,
      status TEXT DEFAULT 'available',
      owner_name TEXT, owner_message TEXT, owner_color TEXT, owner_text TEXT,
      payment_id TEXT, updated_at DATETIME
    );
    CREATE INDEX IF NOT EXISTS idx_status ON slots(status);
  `);
  const stmt = c.env.DB.prepare("INSERT OR IGNORE INTO slots (slot_number, price) VALUES (?, ?)");
  const batch = [];
  for (let i = 1; i <= 10011; i++) {
    const price = i > 9500 ? 5e3 : 100 + i * 0.5;
    batch.push(stmt.bind(i, price));
  }
  await c.env.DB.batch(batch);
  await rebuildCdnCache(c.env);
  return c.json({ message: "Immortal System Initialized", total: 10011 });
});
...
// .wrangler/tmp/bundle-57149l/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-57149l/middleware-loader.entry.ts
