<script>
  ...
  let FIXED_MIN_ZOOM = 0.15; // Will be calculated based on viewport
  ...
  onMount(() => {
    ...
    // Calculate zoom to fit pyramid in fixed container
    const zoomX = containerWidth / canvasWidth;
    const zoomY = containerHeight / canvasHeight;
    FIXED_MIN_ZOOM = Math.min(zoomX, zoomY);
    
    // Ensure zoom doesn't exceed reasonable limits
    FIXED_MIN_ZOOM = Math.min(Math.max(FIXED_MIN_ZOOM, 0.05), 0.3);
    ...
    panzoomInstance = panzoom(svgElement, {
      ...
      minZoom: FIXED_MIN_ZOOM, // Calculated minimum zoom

      initialZoom: FIXED_MIN_ZOOM, // Start at calculated zoom
    ...
    // Add strict bounds constraint to keep pyramid within fixed container
    ...
    panzoomInstance.on('zoom', () => {
      ...
      if (scale < FIXED_MIN_ZOOM) {

        panzoomInstance.zoomAbs(0, 0, FIXED_MIN_ZOOM);
    ...
    // Center pyramid in fixed container
    const offsetX = (containerWidth - canvasWidth * FIXED_MIN_ZOOM) / 2;
    const offsetY = (containerHeight - canvasHeight * FIXED_MIN_ZOOM) / 2;
  ...
  function resetView() {
    ...
    if (panzoomInstance) {
      ...
      const finalZoom = Math.min(Math.max(zoomLevel, 0.05), 0.3);
      

      // Center pyramid in fixed container
      const offsetX = (containerWidth - canvasWidth * finalZoom) / 2;
      const offsetY = (containerHeight - canvasHeight * finalZoom) / 2;
      

      panzoomInstance.zoomAbs(0, 0, finalZoom);
