...
function buildPyramid() {
    ...
    for (let row = 1; row <= CONFIG.ROWS; row++) {
        ...
        for (let col = 0; col < row; col++) {
            let price = 0.1 + ((blockId - 1) * 0.1);
            let type = 'std';
            if (blockId >= 9000) type = 'gold';
...
function handleHover(x, y) {
    ...
    if (id !== state.hoverId) {
        ...
        if (id !== -1 && state.blocks[id]) {
            ...
            if (b.sold) {
                tt.innerHTML = `<div class="tt-label">OWNER</div><span style="color:${b.data.owner_color}">●</span> ${b.data.owner_name}<br><div class="tt-label" style="margin-top:4px">VALUE</div><span style="color:var(--gold); text-shadow: 1px 1px 2px rgba(0,0,0,0.8), -1px -1px 2px rgba(0,0,0,0.8), 1px -1px 2px rgba(0,0,0,0.8), -1px 1px 2px rgba(0,0,0,0.8);">$${b.price.toFixed(2)}</span>`;
                tt.style.borderColor = b.data.owner_color;
            } else {
                tt.innerHTML = `<div class="tt-label">AVAILABLE</div>Block #${b.id} &bull; <span style="color:var(--gold); text-shadow: 1px 1px 2px rgba(0,0,0,0.8), -1px -1px 2px rgba(0,0,0,0.8), 1px -1px 2px rgba(0,0,0,0.8), -1px 1px 2px rgba(0,0,0,0.8);">$${b.price.toFixed(2)}</span>`;
    ...
    if (b.sold) {
                ...
                <div class="prof-header">
                    <div class="prof-header-content">
                        <div class="prof-block-info">
                            ...
                            <div class="prof-block-value">$${parseFloat(b.price || '0').toFixed(2)}</div>
                        </div>
                ...
                <div class="prof-content">
                    <div class="prof-left-section">
                        ...
                        <div class="prof-stats-grid">
                            <div class="stat-card">
                                <span class="stat-val">$${parseFloat(b.price || '0').toFixed(2)}</span>
        ...
        content.innerHTML = `
            ...
            <div class="modal-footer">
                <div style="font-size:1.2rem; font-weight:800; color:var(--gold)">$${b.price.toFixed(2)}</div>
...
function searchInBlock(block, searchTerm) {
    ...
    if (block.price !== undefined && block.price !== null) {
        const priceText = `$${block.price.toFixed(2)}`;
        const priceIntegerText = `$${Math.floor(block.price)}`;
        const priceNumberText = block.price.toString();
        
        // Check for exact price match (e.g., "5" matches $5.00)
